...
use crate::functions::{get_memory_information, send_command_node};
...
#[clap(
    ...
    version,
    about = "This tool mimics the functionality of the docker cli but abstracts
    it over all connected nodes defined in the current users ~/.ssh/config."
)]
...
impl MainParser {
    async fn send_async_command(&self, nodes: &[String], commands: &[String]) -> Vec<String> {
        let bodies = stream::iter(nodes)
            .map(|node| async move { send_command_node(node.clone(), commands).await })
    ...
    pub async fn send_log_command(&self) -> Result<()> {
        match &self.command {
            DockerCommand::Logs { node, container } => {
                ...
                send_command_node_container("logs".to_string(), node.clone(), container.clone())
                    .await?
            }
            _ => {
                error!(
                    "The send_log_command was somehow called with {:#?}",
                    &self.command
                );
    ...
    pub async fn send_stop_command(&self) -> Result<()> {
        match &self.command {
            DockerCommand::Stop { node, container } => {
                debug!("node: {node}, container: {container}");
                send_command_node_container("stop".to_string(), node.clone(), container.clone())
                    .await?
            }
            _ => {
                error!(
    ...
    pub async fn send_rm_command(&self) -> Result<()> {
        match &self.command {
            DockerCommand::Rm { node, container } => {
                debug!("node: {node}, container: {container}");
                send_command_node_container("rm".to_string(), node.clone(), container.clone())
                    .await?
            }
            _ => {
                error!(
    ...
    pub async fn send_inspect_command(&self) -> Result<()> {
        match &self.command {
            DockerCommand::Inspect { node, container } => {
                debug!("node: {node}, container: {container}");
                send_command_node_container("inspect".to_string(), node.clone(), container.clone())
                    .await?
            }
            _ => {
    ...
    pub async fn send_top_command(&self) -> Result<()> {
        match &self.command {
            DockerCommand::Top { node, container } => {
                send_command_node_container("top".to_string(), node.clone(), container.clone())
                    .await?
            }
            _ => {
                error!(
    ...
    pub async fn send_start_command(&self) -> Result<()> {
        match &self.command {
            DockerCommand::Start { node, container } => {
                debug!("node: {node}, container: {container}");
                send_command_node_container("start".to_string(), node.clone(), container.clone())
                    .await?
            }
            _ => {
                error!(
    ...
    pub async fn send_deploy_command(
        &self,
        nodes: &[String],
        project_name: String,
        file: String,
        ...
        let output = session.command("cd").raw_args(suffix).output().await?;
